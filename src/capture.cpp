...
: node_(node),
  ...
  capture_delay_(
    rclcpp::Duration(rclcpp::Duration::from_seconds(node_->get_parameter_or("capture_delay", 0.0))))
{
}
...
void Capture::loadCameraInfo()
{
  ...
  for (int i = 0;; ++i) {
    ...
    if (
      !node_->get_parameter(param_for_code, code) ||
      !node_->get_parameter(param_for_value, value)) {
      break;
    }
  ...
  if (cap_.read(bridge_.image)) {
    ...
    if (info_.height == 0 && info_.width == 0) {
      ...
      if (rescale_camera_info_) {
        ...
        RCLCPP_INFO_ONCE(
          node_->get_logger(), "Camera calibration automatically rescaled from %dx%d to %dx%d",
